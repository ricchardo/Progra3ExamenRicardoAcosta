...
class Hijo : public Padre //creamos la clase Hijo y heredamos de Padre
{
public:
    char letra;
    Hijo(int num, char letra)  //Creamos el Constructor
    {
        setNum(num);
        setLetra(letra);
    }

    void setLetra(char letra) //asignamos la letra
        {
            this->letra = letra;
        }
    char getLetra() //obtenemos el valor que esta en la variable letra
    {
        return letra;
    }
